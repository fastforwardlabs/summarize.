...
from utils import match_most_text, highlight_text
...
@st.cache()
def summarize_text(article, model_name=None):
    model = load_model()
...
model_selection = st.sidebar.selectbox("Choose a summarization model:", ['There', 'Are', 'No','Options'])
...
snippets = match_most_text(summary[0]['summary_text'], article)
